...
#	outDegreeThisNode<-max(1, round(rnorm(1,mean=branching_factor,sd=1)))
	outDegreeThisNode<-5#branching_factor
	if (i>1) outDegreeThisNode<-1
	k<-i+1
	numConn<-0
...
numberOfCombinationEdges<-5	# number of edges across the tree (multiple parents)
...
#	dev.new(width=40, height=100)
	...
	par(mfrow=c(10,3), mar=c(1,1,1,1), las=1, xaxs='i', yaxs='i')
	
	# loop over branching and number of nodes
	for (num_nodes in c(16)){
		for (branching_factor in c(5)){
			for (numberOfCombinationEdges in 0:5*2){	# number of edges across the tree (multiple parents)
...
#					vertex.color = V(tree)$color, vertex.label=NA)
				...
				## add the horizontal and oblique edges
				outWeights<- rep(1,num_nodes) # 1 - nodeDepths/max(nodeDepths)
				inWeights<- rep(1,num_nodes) # nodeDepths/max(nodeDepths)
				#outWeights<-  1.001 - nodeDepths/max(nodeDepths)
				#inWeights<- 0.001 + nodeDepths/max(nodeDepths)

#				outWeights<-  0.001 + nodeDepths/max(nodeDepths)
#				inWeights<- 1.001 - nodeDepths/max(nodeDepths)
				d<-distances(tree)			
						
				### add combination edges
				if (numberOfCombinationEdges>0){
					for (combinationEdge in 1:numberOfCombinationEdges){
						dis<-1
						while (dis==1){
							## pick source node
							outNode<-sample(2:num_nodes, 1, prob=outWeights[-1])
							## pick target node
							inDepth<- sample(nodeDepths[-1], 1, prob=inWeights[-1])
							
							farTarget<- which(d[outNode,] > nodeDepths[outNode])
							inNode<-farTarget
							if (length(farTarget)>1) {
								inNode<-sample(farTarget, 1, prob=inWeights[farTarget])
							}
							thisDistance<-d[outNode,inNode]
							
							if (thisDistance>1){
								tree<-add_edges(tree, c(outNode, inNode))
								d[outNode,inNode]<-1
								d[inNode,outNode]<-1
								dis<-thisDistance
							}
				...
				for (repl in 1:replicateSimulations){
					...
					flush.console()
					print(paste('numNodes=', num_nodes, '  strat=', learningStrategy, '  bf=', branching_factor, '  comb_edges=', numberOfCombinationEdges, '  repl=', repl, sep=''))
...
#					inWeights<- 1.001 - nodeDepths/max(nodeDepths)
					...
					### add combination edges
					if (numberOfCombinationEdges>0){
						for (combinationEdge in 0:numberOfCombinationEdges){
							dis<-1
							while (dis==1){
								## pick source node
								outNode<-sample(2:num_nodes, 1, prob=outWeights[-1])
			...
			#		payoffs<-runif(num_nodes) * (max(nodeDepths) - nodeDepths + 1) # payoffs DEcrease with depth
					...
					summThisSimulation<-c(num_nodes, branching_factor,numberOfCombinationEdges,
						learningStrategy, repl, 
						mean(SLpay[round(timesteps*0.9):timesteps], na.rm=TRUE))
...
names(strategySuccess)<-c('numNodes', 'branching','recombinationEdges', 'learningStrat', 'repl', 'meanPayoff')
rownames(strategySuccess) <- NULL
strategySuccess

## what are the relevant x values?
xx<-unique(strategySuccess$recombinationEdges)
...
plot(1:max(xx), 1:max(xx)*0+1, type='n', ylim=c(0,0.7), xlim=c(0,max(xx)),
	xlab='Recombination edges (total nodes=16)', ylab='Success of social learning strategy')
...
for (learnStrat in 0:4){
	...
	for (x in xx){
#	for (nNodes in xx){
		d<-subset(b, recombinationEdges==x)
